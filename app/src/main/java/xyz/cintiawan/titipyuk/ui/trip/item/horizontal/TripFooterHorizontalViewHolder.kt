...
class TripFooterHorizontalViewHolder(v: View) : RecyclerView.ViewHolder(v) {
...
//        itemView.progress_bar.visibility = if(state == State.LOADING) View.VISIBLE else View.GONE
    ...
    companion object {
        fun create(retry: () -> Unit, parent: ViewGroup): TripFooterHorizontalViewHolder {
            val view = LayoutInflater.from(parent.context)
                    .inflate(R.layout.item_footer_horizontal, parent, false)
            view.btn_retry.setOnClickListener { retry() }

            return TripFooterHorizontalViewHolder(view)
        }
    }
