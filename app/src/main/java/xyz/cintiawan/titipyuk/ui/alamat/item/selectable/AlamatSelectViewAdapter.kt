...
class AlamatSelectViewAdapter(private val retry: () -> Unit, private val onSelect: (AlamatItem) -> Unit) : RecyclerView.Adapter<RecyclerView.ViewHolder>() {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        return when(viewType) {
            DATA_VIEW_TYPE -> {
                val binding: ItemAlamatSelectBinding = DataBindingUtil.inflate(LayoutInflater.from(parent.context), R.layout.item_alamat_select, parent, false)
                AlamatSelectDataViewHolder(binding, onSelect)
            }
            FOOTER_VIEW_TYPE -> AlamatFooterViewHolder.create(retry, parent)
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        when(holder) {
            is AlamatSelectDataViewHolder -> holder.bind(alamats[position])
